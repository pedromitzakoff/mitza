...
-- uma vez. Rode depois de supabase/recurring-tasks.sql e ANTES do Passo 3
-- de supabase/cleanup-legacy-recurring-tasks.sql, pra nunca haver uma
-- janela em que o modelo antigo já parou de gerar e o novo ainda não
-- existe (decisão do usuário, 28/07).
...
begin
  -- Checar saldo — 2x por semana, sem checklist.
  if not exists (select 1 from recurring_tasks where title = 'Checar saldo') then
    ...
    values ('Checar saldo', '💰', '#4169E1', true, true, false, false)
  ...
  -- Reportar cliente — 1x por semana, sem checklist.
  if not exists (select 1 from recurring_tasks where title = 'Reportar cliente') then
    insert into recurring_tasks (title, icon, color, applies_to_all, is_active, has_checklist, uses_account_review)
    values ('Reportar cliente', '📊', '#4169E1', true, true, false, false)
