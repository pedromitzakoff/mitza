...
comment on column ad_creative_daily_metrics.preview_image_url is
  'Representação visual (imagem de capa) do criativo — hoje sempre NULL (sem scraping, sem download, sem cache de mídia). Deliberadamente desacoplado de origem: pode vir do Instagram, CDN do Meta, upload manual, cache próprio ou geração interna — o nome do campo nunca deve sugerir uma fonte específica. Enriquecimento OPCIONAL, nunca parte da identidade do criativo (essa é sempre client_id + creative_name) — NUNCA adicione um NOT NULL nesta coluna: o módulo tem que continuar funcionando (com placeholder) pra qualquer criativo sem preview, pra sempre. A UI já trata null/preenchido; popular este campo futuramente não exige mudança estrutural na interface, só o dado passa a existir.';
