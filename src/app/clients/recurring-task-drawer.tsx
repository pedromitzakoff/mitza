...
export function RecurringTaskDrawer({
  ...
  function renderExecutionRow(execution: RecurringTaskExecutionDetail, dense: boolean) {
    ...
    return (
      <li key={execution.id} className={dense ? "text-sm text-overview-text-primary" : "rounded-md border border-overview-border p-2 text-sm"}>
        <p className={dense ? undefined : "text-overview-text-primary"}>
          <span className="font-medium">{formatDateTime(execution.executedAt)}</span>
          <span className="text-overview-text-secondary"> · {execution.authorName}</span>
        </p>
        ...
        {execution.notes && <p className="mt-0.5 text-xs text-overview-text-secondary">{execution.notes}</p>}
      </li>
    );
  }
    ...
    <>
      ...
      <div className="mitza-panel-in fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col overflow-y-auto border-l border-overview-border bg-overview-surface p-5 shadow-lg">
        <div className="flex items-start justify-between gap-3">
          <h2 className="flex items-center gap-1.5 text-lg font-semibold text-overview-text-primary">
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 rounded-md border border-overview-border bg-overview-surface-subtle p-3 sm:grid-cols-3">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wide text-overview-text-secondary">Esta semana</p>
            <p className="mt-0.5 text-sm text-overview-text-primary">{weekCountLabel}</p>
          </div>
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wide text-overview-text-secondary">Próxima execução</p>
            <p className="mt-0.5 text-sm text-overview-text-primary">{detail.nextExecutionLabel}</p>
          </div>
          {showPreviousWeek && (
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wide text-overview-text-secondary">Semana anterior</p>
              <p className="mt-0.5 text-sm text-overview-text-primary">{previousWeekCountLabel}</p>
              {previousWeek!.pending?.isPending && (
                <p className="mt-0.5 text-xs text-overview-text-secondary">{formatPreviousWeekMissingLine(previousWeek!.pending)}</p>
              )}
            </div>
          )}
        </div>
        ...
        <section className="mt-4 border-t border-overview-border pt-4">
          ...
          {detail.weekExecutions.length > 0 ? (
            ...
            <p className="mt-2 text-sm text-overview-text-secondary">Nenhuma execução registrada nesta semana ainda.</p>
          )}
        </section>
        ...
        <section className="mt-4 border-t border-overview-border pt-4">
          ...
          {detail.history.length > 0 ? (
            ...
            <p className="mt-2 text-sm text-overview-text-secondary">Nenhuma execução registrada ainda.</p>
          )}
        </section>
      </div>
    </>
