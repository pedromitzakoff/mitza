 ...
 *
 * Etapa "Filtro por nome (contém/não contém)": este componente voltou a
 * ficar SEM nenhuma lógica de filtro própria — a primeira versão tinha um
 * controle por tabela, mas o pedido real era UM controle só, no topo do
 * relatório, que escolhe qual das 3 tabelas filtrar (ver
 * `ReportFilterableTables`, que já entrega `table.rows`/`table.emptyMessage`
 * prontos, filtrados ou não). Este componente nunca sabe se está recebendo
 * a lista inteira ou uma versão filtrada — só renderiza `table` como
 * sempre fez, mesmo contrato de antes da etapa do filtro.
 */
...
export function ReportTableSection({ table }: { table: PerformanceReportTable }) {
  ...
  // Etapa "Filtro por nome": também é o estado usado quando o filtro do
  // topo (`ReportFilterableTables`) zera esta tabela — `table.emptyMessage`
  // já vem trocado por "Sem resultado para esse filtro." nesse caso,
  // então este componente nunca precisa saber se está vazio "de verdade"
  // ou "por causa do filtro".
  if (count === 0) {
    return (
