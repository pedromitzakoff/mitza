...
import type { ReportActionItemDependency, ReportActionItemStatus, ReportTimelineEventType } from "@/lib/supabase/database.types";
...
/**
 * Etapa "MITZA 2.0 — Fase G": conteúdo do relatório individual, extraído de
 * `/reports/[clientId]/page.tsx` pra ser renderizado tanto pela rota própria
 * (que continua existindo — os Server Actions abaixo redirecionam pra ela em
 * caso de erro, nunca alterados nesta fase) quanto pela aba "Relatórios" do
 * Cliente (`/clients/[id]`). Nenhuma lógica de dado/ação mudou — só o corpo
 * visual saiu do componente de página pra um componente reutilizável.
 *
 * Não inclui o cabeçalho (nome do cliente, link "Voltar", navegação de mês)
 * — isso é responsabilidade de quem chama, porque o cabeçalho da rota própria
 * e o cabeçalho do Cliente (que já mostra nome/mês) são diferentes.
 */
    ...
    <>
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        <SectionCard
          ...
          action={
            <Link href={`/clients/${clientId}?area=execucao${month ? `&month=${month}` : ""}`} className="text-xs font-medium text-brand hover:underline">
