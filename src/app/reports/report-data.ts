...
import { monthRangeFromParam, shiftMonthParam, sumEffectiveSpend, sumExpectedToDate } from "@/lib/sprint-financials";
...
export async function buildReportViewData(
  ...
  // Dados ao vivo — mesma fonte financeira central de sempre
  // (sumEffectiveSpend/sumExpectedToDate/classifySpendStatus), nunca uma
  // conta paralela.
  const [{ data: sprints }, { data: dailySpend }, { data: tasks }] = await Promise.all([
    supabase
      .from("sprints")
      .select("id, start_date, end_date, planned_spend, spend_source, manual_actual_spend")
      .eq("client_id", clientId)
      .gte("start_date", monthRange.firstDay)
  ...
  const planned = monthSprintRows.reduce((sum, s) => sum + s.planned_spend, 0);
  const actual = sumEffectiveSpend(monthSprintRows, dailySpend ?? []);
  const expectedToDate = sumExpectedToDate(monthSprintRows, today);
